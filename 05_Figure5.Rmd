...
activity_weights<-fread("./analysis_files/MoCHI_weights/3state_FL_weights_Activity.txt")
...
#merge with weighted mean weights and with all
...
cluster_annotations[pocket_cluster==14,site:="MT3-DFG"]
...
for (pocket in unique(pocket_dataset_summarisedbyresidue$pocket_ID)){
  ...
  ggplot(stacked_data[category %in% c("summary","individual mutations"),])+
    ...
    ggsave(paste(paste("./output_files/Figure5pockets_",paste(unique(subset_summary$pocket_cluster),paste(unique(subset_summary$site),pocket,sep="_"),sep="_"),sep = ""),"pdf",sep = "."))
...
#average wmean per pocket, split by pocket cluster
ggplot(pockets_summary)+
  ...
  ggtitle("all pockets")
ggsave("./output_files/FigureS6b_pocketclusters_avgwmean_distributions_FL.pdf")


#druggability vs mutation effects
...
ggplot(pockets_summary)+
  ...
  theme(strip.text = element_text(size=7))+
  facet_wrap(~cluster_anno)
ggsave("./output_files/FigureS6c_pocketclusters_consensus_site_population_vs_avg_wmean_FL.pdf",width=10,height = 7)

ggplot(pockets_summary)+
  geom_point(aes(x=consensus_site_population,y=avg_wmean,col=site),size=1)+
  geom_hline(aes(yintercept=1),linetype="dashed",col="red")+
  geom_vline(aes(xintercept=12),linetype="dashed",col="red")+
  theme(strip.text = element_text(size=7))+
  facet_wrap(~structure)
ggsave("./output_files/FigureS6d_structures_consensus_site_population_vs_avg_wmean_FL.pdf",width=10,height = 7)


#summary of pocket clusters
pockets_summary_persitescores<-pockets_summary[,list(avg_avg_wmean = mean(`avg_wmean`),
                      avg_max_wmean = mean(`max_wmean`),
                      max_avg_wmean = max(`avg_wmean`),
                      npockets= length(`avg_wmean`)),by=cluster_anno]



#summary of sites - druggable pockets
pockets_summary_persitescores_druggable<-pockets_summary[consensus_site_population>12,list(avg_avg_wmean = mean(`avg_wmean`),
                      avg_max_wmean = mean(`max_wmean`),
                      max_avg_wmean = max(`avg_wmean`),
                      max_max_wmean = max(`max_wmean`),
                      npockets= length(`avg_wmean`)),by=cluster_anno]
pockets_summary_persitescores_druggable[order(max_avg_wmean,decreasing = TRUE),c("cluster_anno","max_avg_wmean")]
pockets_summary_persitescores_druggable[order(avg_avg_wmean,decreasing = TRUE),c("cluster_anno","avg_avg_wmean")]
pockets_summary_persitescores_druggable[order(avg_max_wmean,decreasing = TRUE),c("cluster_anno","avg_max_wmean")]


pockets_summary_persitescores_druggable_hot<-pockets_summary[consensus_site_population>12 & avg_wmean>1,list(avg_avg_wmean = mean(`avg_wmean`),
                      avg_max_wmean = mean(`max_wmean`),
                      max_avg_wmean = max(`avg_wmean`),
                      npockets= length(`avg_wmean`)),by=cluster_anno]
pockets_summary_persitescores_druggable_hot[order(npockets,decreasing = TRUE),c("cluster_anno","npockets")]


```

```{r pockets summary - aggregated stats by cluster}
...
pockets_summary_aggregated<-pockets_summary[,list(
         ...
         median_druggability_score_active=median(consensus_site_population[which(structure_nr %in% c("1Y57","1YI6"))]),
         max_druggability_score_active=max(consensus_site_population[which(structure_nr %in% c("1Y57","1YI6"))]),
         median_druggability_score_inactive=median(consensus_site_population[which(structure_nr %in% c("2SRC","1FMK","1KSW"))]),
         max_druggability_score_inactive=max(consensus_site_population[which(structure_nr %in% c("2SRC","1FMK","1KSW"))]),
         ...
         num_targetable_sites_avg1_dr12=length(which(avg_wmean>1 & consensus_site_population>11)),
         num_targetable_sites_max2_dr12=length(which(max_wmean>2 & consensus_site_population>11)),
         num_targetable_sites_act_avg0.5_dr12=length(which(avg_wmean<(-0.5) & consensus_site_population>11)),
         num_targetable_sites_act_max0.5_dr12=length(which(min_wmean<(-0.5) & consensus_site_population>11)),
...
pockets_summary_aggregated[max_druggability_score_active==-Inf,max_druggability_score_active:=NA]
pockets_summary_aggregated[max_druggability_score_inactive==-Inf,max_druggability_score_inactive:=NA]
...
ggsave("./output_files/Figure5b_pocket_overview_median_muteffects_FL.pdf",width = 7,height = 2)
...
theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave("./output_files/Figure5b_pocket_overview_mediandruggability_FL.pdf",width = 7,height = 2)
...
ggplot(pockets_summary_aggregated)+
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave("./output_files/Figure5b_pocket_overview_prevalence_across_structures_FL.pdf",width = 7,height = 2)
...
ggplot(pockets_summary_aggregated)+
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave("./output_files/Figure5b_pocket_overview_state_FL.pdf",width = 7,height = 2)
...
ggsave("./output_files/Figure5b_pocket_overview_fet_or_FL.pdf",width = 7,height = 2)


write.table(pockets_summary_aggregated,
            file="./output_files/pockey_summary.txt",
