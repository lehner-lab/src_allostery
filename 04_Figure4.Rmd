---
title: "04_Figure4_FL_updated.Rmd"
author: "Toni Beltran"
date: "07/09/2025"
...
weights_flkin_sand_activity[,pos_in_uniprot:=Pos_ref+267]


weights_flkin_sand_activity_sasa<-merge(weights_flkin_sand_activity,src_KD_sasa,
                                         by.x = "pos_in_uniprot",
                                         by.y = "pos",all.x = TRUE)
weights_flkin_sand_activity_sasa_anno<-merge(weights_flkin_sand_activity_sasa,src_annotation,
                                         by.x = "pos_in_uniprot",
                                         by.y = "position",all.x = TRUE)
weights_flkin_sand_activity_sasa_anno<-weights_flkin_sand_activity_sasa_anno[id!="WT",]

#add surface annotation to Y530 (didn't get assigned because in the structure is phosphorylated)
weights_flkin_sand_activity_sasa_anno[KD_lobe=="C-terminal tail",core:="surface"]

#add mut_aa 
weights_flkin_sand_activity_sasa_anno$mut_aa<-unlist(lapply(weights_flkin_sand_activity_sasa_anno$id,FUN = function(string){
  return(substr(string,nchar(string),nchar(string)))}))
weights_flkin_sand_activity_sasa_anno$wt_aa<-unlist(lapply(weights_flkin_sand_activity_sasa_anno$id,FUN = function(string){
  return(substr(string,1,1))}))
weights_flkin_sand_activity_sasa_anno$mut_aa<-factor(weights_flkin_sand_activity_sasa_anno$mut_aa,levels=c("S","T","N","Q","D","E","K","R","H","G","P","C","M","A","L","I","V","F","Y","W"))

#define activating and inactivating mutations
weights_flkin_sand_activity_sasa_anno[,z_twotailed:=(abs(`mean_kcal/mol`)-(0.5))/`std_kcal/mol`]
weights_flkin_sand_activity_sasa_anno[,p_twotailed:=2*(1-pnorm(z_twotailed))]
weights_flkin_sand_activity_sasa_anno[,fdr_twotailed:=p.adjust(p_twotailed,method = "fdr")]

weights_flkin_sand_activity_sasa_anno[,class:="no change"]
weights_flkin_sand_activity_sasa_anno[fdr_twotailed<0.1 & `mean_kcal/mol`<0.5,class:="activating"]
weights_flkin_sand_activity_sasa_anno[fdr_twotailed<0.1 & `mean_kcal/mol`>0.5,class:="inactivating"]

weights_flkin_sand_activity_sasa_anno$class<-factor(weights_flkin_sand_activity_sasa_anno$class,
                                                  levels=c("inactivating","no change","activating"))

#count mutation classes per residue
muts_by_residue<-weights_flkin_sand_activity_sasa_anno[, .(activating = length(which(class=="activating")),
...
mean_ddGas<-weights_flkin_sand_activity_sasa_anno[, .(mean_ddGa = mean(abs(`mean_kcal/mol`))), by = pos_in_uniprot]
...
weights_flkin_sand_activity_sasa_anno_distances<-merge(weights_flkin_sand_activity_sasa_anno,mean_ddGas_distance,by="pos_in_uniprot",
                                                all.x=TRUE)
...
#all sites - mean ddGa
summary(fit_exponential_curve(xvector=mean_ddGas_distance[pos_in_uniprot<521 & !is.na(mindistance),]$mindistance,
                      yvector=mean_ddGas_distance[pos_in_uniprot<521 & !is.na(mindistance),]$mean_ddGa,
                      tit="Figure4a_allsites_mean_ddGa"))

mean_ddGas_distance[,predicted:=1.639*exp(-0.063*mindistance)]

#comparison of R2 of exponential decay vs linear model fits
cor(mean_ddGas_distance[pos_in_uniprot<521 & !is.na(mindistance),]$predicted,mean_ddGas_distance[pos_in_uniprot<521 & !is.na(mindistance),]$mean_ddGa,use="pairwise.complete.obs")**2
cor(mean_ddGas_distance[pos_in_uniprot<521 & !is.na(mindistance),]$mindistance,mean_ddGas_distance[pos_in_uniprot<521 & !is.na(mindistance),]$mean_ddGa,use="pairwise.complete.obs")**2

lm(mean_ddGa ~ mindistance,data=mean_ddGas_distance[pos_in_uniprot<521 & !is.na(mindistance),])$coefficients

#active site + allosteric sites - mean ddGa
summary(fit_exponential_curve(xvector=mean_ddGas_distance[pos_in_uniprot<521 & !is.na(mindistance) & res_type!="other",]$mindistance,
...
summary(fit_exponential_curve(xvector=weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance),]$mindistance,
                      yvector=abs(weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance),]$`mean_kcal/mol`),
                      tit="FigureS5a_allsites_allmutations"))

#all mutations - active site + allo sites
summary(fit_exponential_curve(xvector=weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance) & res_type!="other",]$mindistance,
                      yvector=abs(weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance) & res_type!="other",]$`mean_kcal/mol`),
                      tit="FigureS5b_active_allosteric_sites_allmutations"))


#all mutations - n-lobe sites
summary(fit_exponential_curve(xvector=weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<342 & !is.na(mindistance),]$mindistance,
                      yvector=abs(weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<342 & !is.na(mindistance),]$`mean_kcal/mol`),
                      tit="Figure4b_Nlobe_allmutations"))

#all mutations - c-lobe sites
summary(fit_exponential_curve(xvector=weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot>346 & pos_in_uniprot<521 & !is.na(mindistance),]$mindistance,
                      yvector=abs(weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot>346 & pos_in_uniprot<521 & !is.na(mindistance),]$`mean_kcal/mol`),
                      tit="Figure4b_Clobe_allmutations"))

#all mutations - negative ddG
summary(fit_exponential_curve(xvector=weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance) & `mean_kcal/mol`<0,]$mindistance,
                      yvector=abs(weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance)  & `mean_kcal/mol`<0,]$`mean_kcal/mol`),
...
neg_muts<-abs(weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance)  & `mean_kcal/mol`<0,]$`mean_kcal/mol`)
pos_muts<-abs(weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance)  & `mean_kcal/mol`>0,]$`mean_kcal/mol`)

pos_muts_mindistances<-weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance)  & `mean_kcal/mol`>0,]$mindistance
neg_muts_mindistances<-weights_flkin_sand_activity_sasa_anno_distances[pos_in_uniprot<521 & !is.na(mindistance)  & `mean_kcal/mol`<0,]$mindistance
...
ggplot(df_toplot) +  aes(x = var , fill=class)  + geom_histogram( position="identity", alpha=0.5 , bins = 100 )+
    theme_classic()
ggsave("./output_files/FigureS5c_matched_distribution_of_muteffects_posneg.pdf")
fitted_decay_values<-c(fitted_decay_values,
                       fit_exponential_curve(xvector=pos_mut_distances,
                      yvector=pos_mut_sampled,
                      tit="FigureS5d_subsampled_density",plotfig=TRUE,writepar=TRUE)[2])
...
fitted_decay_values<-c(fitted_decay_values,
                       fit_exponential_curve(xvector=pos_mut_distances,
                      yvector=pos_mut_sampled,
                      tit="FigureS5d_subsampled_density",plotfig=FALSE,writepar=TRUE)[2])
}}

median(fitted_decay_values)
max(fitted_decay_values)

(length(which(fitted_decay_values> (0.011)))+1)/10000
...
ggplot(data.table(fitted_decay_values))+
  ...
  geom_vline(aes(xintercept=0.011),col="red",linetype="dashed")+
  theme_classic()
ggsave("./output_files/FigureS5e_distribution_of_decays_subssamples.pdf")
...
library(msir)
weights_flkin_sand_activity_sasa_anno_distances_corrected<-weights_flkin_sand_activity_sasa_anno_distances[!is.na(mindistance) & pos_in_uniprot<521,]
weights_flkin_sand_activity_sasa_anno_distances_corrected<-weights_flkin_sand_activity_sasa_anno_distances_corrected[order(mindistance),]
#fit nonparametric curve
loess_fit<-loess.sd(x=weights_flkin_sand_activity_sasa_anno_distances_corrected$mindistance,
         y=weights_flkin_sand_activity_sasa_anno_distances_corrected$`mean_kcal/mol`)

weights_flkin_sand_activity_sasa_anno_distances_corrected$fitted<-loess_fit$y
weights_flkin_sand_activity_sasa_anno_distances_corrected[,residuals:=`mean_kcal/mol`-fitted]

`%notin%`<-Negate(`%in%`)

ggplot(weights_flkin_sand_activity_sasa_anno_distances_corrected[pos_in_uniprot<518 &
  ...
  theme_classic()
ggsave("./output_files/FigureS5k_distance_corrected_effects_secondary_structure.pdf")


#all mutations - decay at secondary structure elements
weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="",secondary_structure_uniprot:="loop"]
table(weights_flkin_sand_activity_sasa_anno_distances$secondary_structure_uniprot)

summary(fit_exponential_curve(x=weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="helix" & pos_in_uniprot<521 & !is.na(mindistance),]$mindistance,
                      y=abs(weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="helix" & pos_in_uniprot<521 & !is.na(mindistance),]$`mean_kcal/mol`),
                      tit="FigureS5l_alpha helix"))
summary(fit_exponential_curve(x=weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="beta strand" & pos_in_uniprot<521 & !is.na(mindistance),]$mindistance,
                      y=abs(weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="beta strand" & pos_in_uniprot<521 & !is.na(mindistance),]$`mean_kcal/mol`),
                      tit="FigureS5l_beta strand"))
summary(fit_exponential_curve(x=weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="turn" & pos_in_uniprot<521 & !is.na(mindistance),]$mindistance,
                      y=abs(weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="turn" & pos_in_uniprot<521 & !is.na(mindistance),]$`mean_kcal/mol`),
                      tit="FigureS5l_turn"))
summary(fit_exponential_curve(x=weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="loop" & pos_in_uniprot<521 & !is.na(mindistance),]$mindistance,
                      y=abs(weights_flkin_sand_activity_sasa_anno_distances[secondary_structure_uniprot=="loop" & pos_in_uniprot<521 & !is.na(mindistance),]$`mean_kcal/mol`),
                      tit="FigureS5l_loop"))
...
geom_density(aes(x=sampled_distances),bw=2)+
  theme_classic()
ggsave("./output_files/FigureS5f_pairwisedistances_distributions.pdf")
...
geom_vline(aes(xintercept=median(distmat_melted[pos1 %in% c(allosteric_all,active_site) & pos2 %in% c(allosteric_all,active_site) & pos2>pos1]$distance)),col="red")+
  theme_classic()
ggsave("./output_files/FigureS5g_pairwisedistances_median_distributions.pdf")
...
geom_density(aes(x=sampled_mindistances),bw=2)+
  theme_classic()
ggsave("./output_files/FigureS5h_mindistances_distributions.pdf")
...
geom_vline(aes(xintercept=median_real),col="red")+
  theme_classic()
ggsave("./output_files/FigureS5i_mindistances_median.pdf")
...
weights_flkin_sand_activity_sasa_anno_distances_projected<-merge(weights_flkin_sand_activity_sasa_anno_distances,projected_distances_min,
                                                     by="pos_in_uniprot")

#min
ggplot(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & abs(ydistance_min)<5 & abs(zdistance_min)<5,])+
  geom_point(aes(x=xdistance_min,`mean_kcal/mol`))
ggplot(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & abs(xdistance_min)<5 & abs(zdistance_min)<5,])+
  geom_point(aes(x=ydistance_min,`mean_kcal/mol`))
ggplot(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & abs(xdistance_min)<5 & abs(ydistance_min)<5,])+
...
#x
summary(fit_exponential_curve(x=abs(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & xdistance_min>-2 & abs(ydistance_min)<side & abs(zdistance_min)<side,]$xdistance_min),
                      y=abs(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & xdistance_min>-2 & abs(ydistance_min)<side & abs(zdistance_min)<side,]$`mean_kcal/mol`),
...
#y
summary(fit_exponential_curve(x=abs(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & ydistance_min>-2 & abs(zdistance_min)<side & abs(xdistance_min)<side,]$ydistance_min),
                      y=abs(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & ydistance_min>-2 & abs(zdistance_min)<side & abs(xdistance_min)<side,]$`mean_kcal/mol`),
...
#z
summary(fit_exponential_curve(x=abs(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & zdistance_min>-2 & abs(ydistance_min)<side & abs(xdistance_min)<side,]$zdistance_min),
                      y=abs(weights_flkin_sand_activity_sasa_anno_distances_projected[pos_in_uniprot<521 & zdistance_min>-2 & abs(ydistance_min)<side & abs(xdistance_min)<side,]$`mean_kcal/mol`),
...
ggplot()+
  geom_line(aes(x=seq(0:25),y=expdecay(1.028,-0.215,seq(0,25))),col="red")+
  geom_line(aes(x=seq(0:25),y=expdecay(1.002,-0.062,seq(0,25))),col="red",linetype="dashed")+
  geom_line(aes(x=seq(0:25),y=expdecay(0.941,-0.035,seq(0,25))),col="blue")+
  geom_line(aes(x=seq(0:25),y=expdecay(0.931,-0.082,seq(0,25))),col="blue",linetype="dashed")+
  geom_line(aes(x=seq(0:25),y=expdecay(1.043,-0.045,seq(0,25))),col="orange")+
  geom_line(aes(x=seq(0:25),y=expdecay(1.049,-0.074,seq(0,25))),col="orange",linetype="dashed")+
...
activity_weights<-data.table(weights_flkin_sand_activity)
...
```{r contacts, activating mutations}

#activating
plot_mut_effects_contacts_activating<-function(distribution,contact_type){

activity_weights<-data.table(weights_flkin_sand_activity)
activity_weights<-activity_weights[`mean_kcal/mol`<0,]
...
contact_plot<-ggplot(activity_weights[Pos_ref<518 & contact != "no contact",])+
  geom_jitter(aes(x=contact,y=`mean_kcal/mol`,col=contact),size=2)+
  geom_violin(aes(x=contact,y=`mean_kcal/mol`),col="black",fill=NA)+
  geom_boxplot(aes(x=contact,y=`mean_kcal/mol`),col="black",fill=NA,width=0.07,outlier.shape = NA)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
        ggtitle(paste("activity",contact_type,distribution,sep=" "))+
  scale_color_manual(values=alpha(c("red","orange","blue","purple"),0.5))
  ggsave(paste(paste(paste("./output_files/Figure4f_contacts_activating",contact_type,sep = "_"),distribution,sep = "_"),"pdf",sep="."),plot=contact_plot,
         width = 8.5,height = 5)
...
sb_contact_classification_activating<-plot_mut_effects_contacts_activating("KD_KD","sb")
pc_contact_classification_activating<-plot_mut_effects_contacts_activating("KD_KD","pc")
hbsb_contact_classification_activating<-plot_mut_effects_contacts_activating("KD_KD","hbsb")
hbss_contact_classification_activating<-plot_mut_effects_contacts_activating("KD_KD","hbss")



#inactivating
plot_mut_effects_contacts_inactivating<-function(distribution,contact_type){
...
contact_plot<-ggplot(activity_weights[Pos_ref<518 & contact != "no contact",])+
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
        ggtitle(paste("activity",contact_type,distribution,sep=" "))+
  scale_color_manual(values=alpha(c("red","orange","blue","purple"),0.5))
  ggsave(paste(paste(paste("./output_files/Figure4f_contacts_inactivating",contact_type,sep = "_"),distribution,sep = "_"),"pdf",sep="."),plot=contact_plot,
         width = 8.5,height = 5)
...
sb_contact_classification_inactivating<-plot_mut_effects_contacts_inactivating("KD_KD","sb")
pc_contact_classification_inactivating<-plot_mut_effects_contacts_inactivating("KD_KD","pc")
hbsb_contact_classification_inactivating<-plot_mut_effects_contacts_inactivating("KD_KD","hbsb")
hbss_contact_classification_inactivating<-plot_mut_effects_contacts_inactivating("KD_KD","hbss")



#calculate enrichment of mutation classes in contact types
table(weights_flkin_sand_activity_sasa_anno$class)

sb_contact_classification_mutclasses<-merge(sb_contact_classification,weights_flkin_sand_activity_sasa_anno,
                                            by.x="Pos_ref",by.y="pos_in_uniprot")
table(sb_contact_classification_mutclasses$sb_contact,
      sb_contact_classification_mutclasses$class)

pc_contact_classification_mutclasses<-merge(pc_contact_classification,weights_flkin_sand_activity_sasa_anno,
                                            by.x="Pos_ref",by.y="pos_in_uniprot")
table(pc_contact_classification_mutclasses$pc_contact,
      pc_contact_classification_mutclasses$class)

hbss_contact_classification_mutclasses<-merge(hbss_contact_classification,weights_flkin_sand_activity_sasa_anno,
                                            by.x="Pos_ref",by.y="pos_in_uniprot")
table(hbss_contact_classification_mutclasses$hbss_contact,
      hbss_contact_classification_mutclasses$class)

hbsb_contact_classification_mutclasses<-merge(hbsb_contact_classification,weights_flkin_sand_activity_sasa_anno,
                                            by.x="Pos_ref",by.y="pos_in_uniprot")
table(hbsb_contact_classification_mutclasses$hbsb_contact,
      hbsb_contact_classification_mutclasses$class)
...
table(weights_flkin_sand_activity_sasa_anno$mut_aa)
table(weights_flkin_sand_activity_sasa_anno[class=="activating",]$mut_aa)
table(weights_flkin_sand_activity_sasa_anno[class=="inactivating",]$mut_aa)

weights_flkin_sand_activity_sasa_anno$mut_aa<-factor(weights_flkin_sand_activity_sasa_anno$mut_aa,
                                            levels=strsplit("DEKRHSTQNCGPAMLIVFYW","")[[1]])


active_site<-c(277, 279, 281, 282, 284, 296, 298, 389, 391, 394, 396, 407, 428)

mut_aa_counts_by_class<-data.table(t(rbind(tabulate(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %notin% active_site,]$mut_aa), tabulate(weights_flkin_sand_activity_sasa_anno[class=="activating" & pos_in_uniprot %notin% active_site,]$mut_aa), tabulate(weights_flkin_sand_activity_sasa_anno[class=="inactivating" & pos_in_uniprot %notin% active_site,]$mut_aa))))
colnames(mut_aa_counts_by_class)<-c("all_mutations","activating","inactivating")
mut_aa_counts_by_class$mut_aa<-names(table(weights_flkin_sand_activity_sasa_anno$mut_aa))
...
fisher.test(cbind(c(56,1883),c(109-56,4755-1883)))
#hydrophobics activating as a group, OR=1.61, p=0.01717

```

```{r activating and inactivating mutations - wt aa enrichments}

table(weights_flkin_sand_activity_sasa_anno$wt_aa)
table(weights_flkin_sand_activity_sasa_anno[class=="activating",]$wt_aa)
table(weights_flkin_sand_activity_sasa_anno[class=="inactivating",]$wt_aa)

weights_flkin_sand_activity_sasa_anno$wt_aa<-factor(weights_flkin_sand_activity_sasa_anno$wt_aa,
                                            levels=strsplit("DEKRHSTQNCGPAMLIVFYW","")[[1]])

wt_aa_counts_by_class<-data.table(t(rbind(tabulate(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %notin% active_site,]$wt_aa), tabulate(weights_flkin_sand_activity_sasa_anno[class=="activating" & pos_in_uniprot %notin% active_site,]$wt_aa), tabulate(weights_flkin_sand_activity_sasa_anno[class=="inactivating" & pos_in_uniprot %notin% active_site,]$wt_aa))))
colnames(wt_aa_counts_by_class)<-c("all_mutations","activating","inactivating")
wt_aa_counts_by_class$wt_aa<-names(table(weights_flkin_sand_activity_sasa_anno$wt_aa))
...
weights_flkin_sand_activity_sasa_anno_distances_contactclass<-merge(weights_flkin_sand_activity_sasa_anno_distances,contact_classification_all,by.x="pos_in_uniprot", by.y="Pos_ref")
...
r2<-c(tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~secondary_structure_uniprot)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~mut_aa)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~sb_contact+pc_contact+hbss_contact+hbsb_contact)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~wt_aa)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~rsasa_all)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_catD)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP+distance_catD+wt_aa+sb_contact+pc_contact+hbss_contact+hbsb_contact+mut_aa+rsasa_all+secondary_structure_uniprot)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP+distance_catD+wt_aa+sb_contact+pc_contact+hbss_contact+hbsb_contact+mut_aa+rsasa_all+secondary_structure_uniprot+secondary_structure)))
...
tenfold_xval_plot(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP+distance_catD+wt_aa+sb_contact+pc_contact+hbss_contact+hbsb_contact+mut_aa+rsasa_all+secondary_structure_uniprot+secondary_structure))
...
#merge esm1b/thermompnn

src_thermoMPNN<-fread("analysis_files/ThermoMPNN_inference_2SRC.csv")
src_thermoMPNN[,pos_in_uniprot:=position+87]
src_thermoMPNN[,mutant:=paste(wildtype,pos_in_uniprot,mutation,sep="")]

src_esm1b<-fread("analysis_files/SRC (SRC) _ P12931.csv")

weights_flkin_sand_activity_sasa_anno_distances_contactclass[,mutant:=paste(wt_aa,pos_in_uniprot,mut_aa,sep="")]

weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds<-merge(merge(weights_flkin_sand_activity_sasa_anno_distances_contactclass,src_thermoMPNN[,c("mutant","ddG_pred")],by="mutant"),
      src_esm1b[,c("variant","score")],by.x="mutant",by.y="variant")


#plot esm1b, thermo
ggplot(weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds)+
  geom_hex(aes(x=ddG_pred,y=`mean_kcal/mol`),bins=70)+
  scale_fill_viridis()+
  theme_classic()

cor(weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds$ddG_pred,
    weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds$`mean_kcal/mol`,
    use="pairwise.complete.obs")

ggplot(weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds)+
  geom_hex(aes(x=score,y=`mean_kcal/mol`),bins=70)+
  scale_fill_viridis()+
  theme_classic()

cor(weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds$score,
    weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds$`mean_kcal/mol`,
    use="pairwise.complete.obs")

tenfold_xval_plot(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP+distance_catD+wt_aa+sb_contact+pc_contact+hbss_contact+hbsb_contact+mut_aa+rsasa_all+secondary_structure_uniprot+secondary_structure+
                                                                                                                                                                             ddG_pred+score))
tenfold_xval_plot(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP+distance_catD+wt_aa+sb_contact+pc_contact+hbss_contact+hbsb_contact+mut_aa+rsasa_all+secondary_structure_uniprot+secondary_structure))

tenfold_xval_plot(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass_preds[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP+distance_catD+ddG_pred+score))
#just distance + predictors not as good either


#adding esm and ddG_pred does not provide additional information
...
```{r linear models - transformed distance}

weights_flkin_sand_activity_sasa_anno_distances_contactclass<-merge(weights_flkin_sand_activity_sasa_anno_distances,contact_classification_all,by.x="pos_in_uniprot", by.y="Pos_ref")

weights_flkin_sand_activity_sasa_anno_distances_contactclass[,distance_ATP_exp:=exp(-0.063*distance_ATP)]
weights_flkin_sand_activity_sasa_anno_distances_contactclass[,distance_catD_exp:=exp(-0.063*distance_catD)]
...
r2<-c(tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~secondary_structure_uniprot)),
      ...
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~wt_aa)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~rsasa_all)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP_exp)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_catD_exp)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP_exp+distance_catD_exp+wt_aa+sb_contact+pc_contact+hbss_contact+hbsb_contact+mut_aa+rsasa_all+secondary_structure_uniprot)),
      tenfold_xval(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP_exp+distance_catD_exp+wt_aa+sb_contact+pc_contact+hbss_contact+hbsb_contact+mut_aa+rsasa_all+secondary_structure_uniprot+secondary_structure)))


names<-c("secondary structure type", "mutant aa","contacts","wt aa","rsasa","exp(distance to nucleotide)","exp(distance to catalytic D)","all","all plus specific secondary structure")
toplot_r2s<-data.table(r2,names=rep(names, each = 2),statistic=rep(c("mean","sd"),times=9))
toplot_r2s$names<-factor(toplot_r2s$names,levels=names)
...
ggplot(merged_toplot)+
  ...
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
ggsave("./output_files/Figure4h_R2_comparison_exp.pdf")
...
tenfold_xval_plot(data=data.frame(weights_flkin_sand_activity_sasa_anno_distances_contactclass[pos_in_uniprot<518 & pos_in_uniprot %notin% active_site,]),formula=as.formula(`mean_kcal.mol`~distance_ATP_exp+distance_catD_exp+wt_aa+sb_contact+pc_contact+hbss_contact+hbsb_contact+mut_aa+rsasa_all+secondary_structure_uniprot+secondary_structure))
ggsave("./output_files/Figure4i_obs_vs_predicted_all_exp.pdf")
