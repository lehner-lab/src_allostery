...
weights_flkin_sand_abundance_sasa_anno$mut_aa<-factor(weights_flkin_sand_abundance_sasa_anno$mut_aa,levels=c("S","T","N","Q","D","E","K","R","H","G","P","C","M","A","L","I","V","F","Y","W"))
...
ggplot(weights_flkin_sand_activity_sasa_anno[cat_regions=="ATP binding",])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3b_ATPbinding_FL_activity.pdf")


ggplot(weights_flkin_sand_activity_sasa_anno[cat_regions=="catalytic loop",])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3b_catloop_FL_activity.pdf")


ggplot(weights_flkin_sand_activity_sasa_anno[cat_regions_sub=="activation segment - Mg2+ positioning loop",])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3b_mg2_FL_activity.pdf")


ggplot(weights_flkin_sand_activity_sasa_anno[cat_regions_sub=="activation segment - alphaEF/substrate binding",])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3b_substratepositioning_loop_FL_activity.pdf")
...
ggplot(weights_flkin_sand_activity_sasa_anno)+
...
sum(table(weights_flkin_sand_activity_sasa_anno$class))

length(unique(weights_flkin_sand_activity_sasa_anno[class !="no change",]$pos_in_uniprot))


active_site_residues<-c(277,279,281,282,284,296,298,389,391,394,396,407,428)

#enrichment in active site
#direct contacts
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% active_site_residues,]$class)
test<-fisher.test(cbind(c(4041+109-38,961-209),c(38,209)))
test$p.value
test$estimate
#atp binding
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% c(277:285,287,295:298),]$class)
test<-fisher.test(cbind(c(4041+109-156,961-139),c(156,110)))
test$p.value
test$estimate
#cat loop
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% c(385:397),]$class)
test<-fisher.test(cbind(c(4041+109-113,961-134),c(113,134)))
test$p.value
test$estimate
#mg2 loop
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% c(407:411),]$class)
test<-fisher.test(cbind(c(4041+109-8,961-87),c(8,87)))
test$p.value
test$estimate
#substrate pos loop
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% c(427:435),]$class)
test<-fisher.test(cbind(c(4041+109-70,961-101),c(70,101)))
test$p.value
test$estimate

#mutations outside active site
`%notin%` <- Negate(`%in%`)
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %notin% active_site_residues,]$class)

length(unique(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %notin% active_site_residues & class!="no change",]$pos_in_uniprot))
...
mean_ddGas[pos_in_uniprot %in% active_site_residues, active_site:=TRUE]
...
muts_by_residue[pos_in_uniprot %in% active_site_residues, active_site:=TRUE]
...
muts_by_residue$fet_or<-unlist(apply(muts_by_residue,MARGIN=1,FUN=function(row){
  ...
  test<-fisher.test(cbind(c(4041-no_change,961+109-change),c(no_change,change)))
...
table(muts_by_residue[fet_fdr<0.1 & fet_or>2 & pos_in_uniprot<347,]$active_site)
table(muts_by_residue[fet_fdr<0.1 & fet_or>2 & pos_in_uniprot>347,]$active_site)
...
from_table_to_structure(muts_by_residue,-3,"3state_FLkin_FLsand","2src")
from_table_to_structure(muts_by_residue,-3,"3state_FLkin_FLsand","1Y57")
...
sd_ddGas<-weights_flkin_sand_activity_sasa_anno[, .(sd_ddGa = sd(abs(`mean_kcal/mol`))), by = pos_in_uniprot]
...
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% foda_network,]$class)
test<-fisher.test(cbind(c(4041+109-59,961-207),c(59,207)))
test$p.value
...
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% foda_network_noAS,]$class)
test<-fisher.test(cbind(c(4041+109-54,961-155),c(54,155)))
test$p.value

foda_network_noAS_nosecondshell<-mean_ddGas_distance[pos_in_uniprot %in% foda_network & res_type=="allosteric",]$pos_in_uniprot
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% foda_network_noAS_nosecondshell,]$class)
test<-fisher.test(cbind(c(4041+109-24,961-24),c(14,24)))
test$p.value

ggplot(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% foda_network,])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3f_fodanetwork_FL_activity.pdf")
...
#inactivating
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot<342 & !(pos_in_uniprot %in% active_site),]$class)
table(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot>346 & !(pos_in_uniprot %in%  active_site),]$class)
fisher.test(cbind(c(163,1097+13),c(581,2819+96)))


muts_by_residue$fet_or_inact<-unlist(apply(muts_by_residue,MARGIN=1,FUN=function(row){
  inact<-as.numeric(row[3])
  no_inact<-19-inact
  test<-fisher.test(cbind(c(4041+109-no_inact,961-inact),c(no_inact,inact)))
  return(test$estimate)
}))
...
#fisher's exact test
enrichment_by_2ary_structure<- data.table(rbind(table(weights_flkin_sand_activity_sasa_anno$secondary_structure,weights_flkin_sand_activity_sasa_anno$class)))
enrichment_by_2ary_structure$secondary_structure<-rownames(rbind(table(weights_flkin_sand_activity_sasa_anno$secondary_structure,weights_flkin_sand_activity_sasa_anno$class)))
...
ggsave("./output_files/Figure3g_mutation_classes_FL.pdf")
...
ggplot(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% c(328,317,408,387),])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3h_Rspine_FL_activity.pdf")

ggplot(weights_flkin_sand_abundance_sasa_anno[pos_in_uniprot %in% c(328,317,408,387),])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
  ...
  theme_classic()
ggsave("./output_files/Figure3i_Rspine_FL_abundance.pdf")
  

ggplot(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% c(284,296,284,395,396,397,349,454,458),])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3h_Cspine_FL_activity.pdf")

ggplot(weights_flkin_sand_abundance_sasa_anno[pos_in_uniprot %in% c(284,296,284,395,396,397,349,454,458),])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_abundance_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_abundance_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_abundance_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_abundance_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3i_Cspine_FL_abundance.pdf")


ggplot(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% 406:435,])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/Figure3j_activationloop_activity_FL.pdf")
...
muts_by_residue$fet_or_act<-unlist(apply(muts_by_residue,MARGIN=1,FUN=function(row){
  ...
  no_act<-19-act
  test<-fisher.test(cbind(c(4041+961-no_act,109-act),c(no_act,act)))
  return(test$estimate)
}))
...
table(muts_by_residue[fet_or_act>1 & fet_p_act<0.1,]$active_site)
...
ggplot(weights_flkin_sand_activity_sasa_anno[pos_in_uniprot %in% act_sites_or1_p0.05,])+
  geom_tile(aes(x=factor(pos_in_uniprot),y=mut_aa,fill=`mean_kcal/mol`))+
    scale_fill_gradientn(colours = c("blue","lightgrey","red"), 
                         values = rescale(c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),0,max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`))),
                         guide = "colorbar", limits=c(min(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`),max(weights_flkin_sand_activity_sasa_anno$`mean_kcal/mol`)))+
  theme_classic()
ggsave("./output_files/FigureS4_activatingsites_FL_activity.pdf")
...
```{r comparing stability and activity}



merged_ddgs_fl<-merge(weights_flkin_sand_abundance,
                      weights_flkin_sand_activity,
      by="id")


merged_ddgs_fl[`mean_kcal/mol.x`>0.5,]
merged_ddgs_fl[`mean_kcal/mol.y`>0.5,]

merged_ddgs_fl[,fdr_destab:=p.adjust(1-pnorm((`mean_kcal/mol.x`-0.5)/(`std_kcal/mol.x`)),method="fdr")]
merged_ddgs_fl[,fdr_inact:=p.adjust(1-pnorm((`mean_kcal/mol.y`-0.5)/(`std_kcal/mol.y`)),method="fdr")]

merged_ddgs_fl[,fdr_stab:=p.adjust(pnorm((`mean_kcal/mol.x`-(-0.5))/(`std_kcal/mol.x`)),method="fdr")]
nrow(merged_ddgs_fl[fdr_stab<0.1,])

nrow(merged_ddgs_fl[fdr_destab<0.1,])
nrow(merged_ddgs_fl[fdr_inact<0.1,])
nrow(merged_ddgs_fl[fdr_inact<0.1 & fdr_destab<0.1,])

merged_ddgs_fl[fdr_destab<0.1,class:="destabilizing"]
merged_ddgs_fl[fdr_inact<0.1,class:="inactivating"]
merged_ddgs_fl[fdr_destab<0.1 & fdr_inact<0.1,class:="both"]

ggplot(merged_ddgs_fl)+
  geom_point(aes(x=`mean_kcal/mol.x`,
                 y=`mean_kcal/mol.y`,
                 col=class))+
  theme_classic()+
  xlab("ddGf")+
  ylab("ddGa")
ggsave("./output_files/Figure_S3_folding_vs_activity.pdf")

table(merged_ddgs_fl$class)
